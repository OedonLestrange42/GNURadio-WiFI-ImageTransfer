...
import math
from .module_original.encoder_seq_shared_only import Encoder_PE
from .module_original.decoder_seq_shared_only import Decoder_PE
...
class DMANet(nn.Module):
    def __init__(self, envs, img_size=256, compressed_channel=128, P=1, CSI_bound=30, withIRS=False, optimizeIRS=True, device=torch.device('cpu')):
        ...
        self.shared_encoder = Encoder_PE(compressed_channel=compressed_channel,
                                         device=self.device)
        self.shared_decoder = Decoder_PE(compressed_channel=compressed_channel,
